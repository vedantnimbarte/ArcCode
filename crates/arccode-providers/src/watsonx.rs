        ...
        match &self.credential {
            ...
            WatsonxCredential::ApiKey(key) => {
                ...
                let resp = self
                    ...
                    .send()
                    .await
                    .map_err(|e| ArccodeError::Provider(format!("watsonx iam: {e}")))?;
                ...
                let body: Value = resp
                    .json()
                    .await
                    .map_err(|e| ArccodeError::Provider(format!("watsonx iam decode: {e}")))?;
                let token = body
                    .get("access_token")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ArccodeError::Provider("watsonx iam: missing access_token".into())
                    })?
                    .to_string();
...
impl Provider for WatsonxProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<ProviderEventStream> {
        ...
        tracing::debug!(target: "arccode::watsonx", "request: {body}");
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ArccodeError::Provider(format!("watsonx request: {e}")))?;
        ...
        let stream = async_stream::try_stream! {
            while let Some(item) = events.next().await {
                ...
                let chunk: Value = match serde_json::from_str(&evt.data) {
                    Ok(v) => v,
                    Err(e) => {
                        tracing::warn!(target: "arccode::watsonx", "bad sse json: {e}: {}", evt.data);
                        continue;
                    }
                };
