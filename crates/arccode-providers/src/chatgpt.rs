...
//! access token obtained via the PKCE browser flow (`arccode auth login`).
//!
...
//! `"chatgpt_refresh"`.  Both are managed by `arccode-cli`'s runtime and OAuth
...
impl Provider for ChatGptProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<ProviderEventStream> {
        ...
        tracing::debug!(target: "arccode::chatgpt", "request: {body}");
        ...
        if !status.is_success() {
            ...
            if status.as_u16() == 401 {
                return Err(ArccodeError::Provider(
            ...
            while let Some(item) = events.next().await {
                ...
                match ev_type {
                    ...
                    "response.failed" | "error" => {
                        ...
                        Err(ArccodeError::Provider(format!("chatgpt: {msg}")))?;
                    }

                    _ => {
                        tracing::trace!(target: "arccode::chatgpt", "unhandled event type: {ev_type}");
                    }
                }
            }
        };
