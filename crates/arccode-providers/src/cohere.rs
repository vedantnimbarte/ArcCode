...
impl Provider for CohereProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<ProviderEventStream> {
        ...
        tracing::debug!(target: "arccode::cohere", "request: {body}");
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ArccodeError::Provider(format!("cohere request: {e}")))?;
        ...
        let stream = async_stream::try_stream! {
            while let Some(item) = events.next().await {
                ...
                let chunk: Value = match serde_json::from_str(&evt.data) {
                    Ok(v) => v,
                    Err(e) => {
                        tracing::warn!(target: "arccode::cohere", "bad sse json: {e}: {}", evt.data);
                        continue;
                    }
                };
