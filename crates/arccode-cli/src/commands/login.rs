//! `arccode login` / `arccode logout` — headless credential management.
//!
...
//! [`arccode_config::secrets`], and the provider table from the TUI modal so
//! the two surfaces stay in lockstep.

use anyhow::{anyhow, Context, Result};
use arccode_config::{ensure_global_dir, global_config_path, secrets, Config};
use arccode_tui::modal::login::PROVIDERS;
use arccode_tui::modal::{LoginPayload, LoginTask};
use std::io::{IsTerminal, Write};
use std::process::ExitCode;

/// Parsed `arccode login` arguments.
...
pub async fn run(opts: LoginOptions) -> Result<ExitCode> {
    ...
    let Some(provider_id) = opts.provider.as_deref() else {
        eprintln!("arccode login: missing PROVIDER.\n");
        print_providers();
    ...
    let spec = PROVIDERS
        ...
        .ok_or_else(|| {
            anyhow!("unknown provider '{provider_id}' — run `arccode login --list` to see options")
        })?;
...
/// `arccode logout <provider>` — remove the keyring entry for a provider.
...
fn print_providers() {
    println!("Usage: arccode login <PROVIDER> [--api-key <KEY>] [--model <MODEL>]");
    println!("       arccode login --list");
    println!("       arccode logout <PROVIDER>\n");
