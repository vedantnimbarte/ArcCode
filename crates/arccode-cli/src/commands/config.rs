...
async fn init(force: bool) -> Result<ExitCode> {
    ...
    if path.exists() && !force {
        eprintln!(
            "arccode: refusing to overwrite existing config at {} (re-run with --force)",
            path.display()
        );
    ...
    let preface = "# arccode starter config — edit freely.\n\
                   # Values support ${ENV_VAR} placeholders that resolve at load time.\n\
                   # Resolution order: defaults < this file < <project>/.arccode/config.toml < env < CLI flags.\n\n";
