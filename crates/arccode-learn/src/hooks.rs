//! The [`LearnHook`] is the concrete `arccode_core::LearningHook` impl that
//! wires memory + stats + session-indexing into the agent loop.
//!
//! The trait itself lives in `arccode-core` (so the loop can call it without
//! a circular dep); this module is the production wiring.

use std::path::PathBuf;
use std::sync::{Arc, Mutex};

use arccode_core::{ContentBlock, LearningHook, Message, Role};
