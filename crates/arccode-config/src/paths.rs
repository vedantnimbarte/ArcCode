...
//!
//! Global:  `~/.arccode/`            (config, credentials, model cache, logs)
//! Project: `<project>/.arccode/`    (sessions, repo index, project overrides)
//!
...
//! directory or a `.arccode` directory. If neither marker is found, the
//! start dir itself is treated as the project root. The home directory is
//! excluded because the global `~/.arccode/` would otherwise be mistaken
...
/// Returns `~/.arccode/`. Pure path computation — does **not** create.
pub fn global_dir() -> Result<PathBuf, ConfigError> {
    Ok(home()?.join(".arccode"))
}

/// Returns `~/.arccode/`, creating it on demand.
...
/// Returns `<project>/.arccode/`. Pure path computation.
pub fn project_dir(project_root: &Path) -> PathBuf {
    project_root.join(".arccode")
}

/// Walks up from `start` looking for `.git` or `.arccode`. The user's home
/// directory is never returned as a project root (the global `~/.arccode/`
...
pub fn find_project_root(start: &Path) -> PathBuf {
    ...
    loop {
        ...
        if !is_home && (cursor.join(".git").exists() || cursor.join(".arccode").exists()) {
...
mod tests {
    ...
    fn project_paths_from_root_layout() {
        ...
        assert!(pp.dir.ends_with(".arccode"));
    ...
    fn find_project_root_finds_marker() {
        let tmp = std::env::temp_dir().join(format!("arccode-test-{}", std::process::id()));
        let nested = tmp.join("a").join("b");
        std::fs::create_dir_all(&nested).unwrap();
        std::fs::create_dir_all(tmp.join(".arccode")).unwrap();
