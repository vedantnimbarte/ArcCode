...
//!   replaying the log.
//!
//! Both live under `<project>/.wingman/autonomous/<run-id>/`.
...
/// markdown files at `~/.wingman/agents/<role>.md`; the variants here are the
...
pub struct Agent {
    ...
    /// `<project>/.wingman/sessions/`. Lets `wingman session fork` operate on
