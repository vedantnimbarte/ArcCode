...
//!
//! `wingman pilot status` prints a flat ASCII version; `wingman pilot
//! watch` reuses the same shape under crossterm; a future wingman-tui
...
//! in-process (orchestrator + dashboard in the same `wingman` invocation)
//! and cross-process (background `wingman pilot` + foreground
//! `wingman pilot watch`) layouts — there is no in-process broadcast
//! requirement.
...
impl DashboardView {
    /// Lay out the dashboard as a 2-column ASCII grid for `wingman pilot
...
/// Discover every run on disk under `<project>/.wingman/autonomous/`,
/// most recent first.
pub fn list_runs(project_root: &Path) -> Result<Vec<RunSummary>, DashboardError> {
    let dir = project_root.join(".wingman").join("autonomous");
    if !dir.exists() {
        return Ok(Vec::new());
    }
...
/// `<project>/.wingman/autonomous/`. Corrupt or unreadable snapshots are
...
/// Mtime-based "did the run change?" probe. Used by `wingman pilot
...
mod tests {
    ...
    fn header_reports_running_count_and_branch() {
        ...
        assert_eq!(model.header.branch, "wingman/auto/2026-05-27-1430-abc");
    ...
    async fn list_runs_finds_state_files_and_orders_by_mtime() {
        ...
        let mut s1 = RunStore::create(
            project.join(".wingman/autonomous/run-a"),
            "run-a",
            "goal A",
            "deadbeef",
            "wingman/auto/run-a",
        ...
        let _s2 = RunStore::create(
            project.join(".wingman/autonomous/run-b"),
            "run-b",
            "goal B",
            "deadbeef",
            "wingman/auto/run-b",
    ...
    /// fresh state shows the new content. This is what `wingman pilot
    /// watch`'s polling loop actually does.
    #[tokio::test]
    async fn dashboard_observes_state_changes_via_mtime() {
        use crate::store::RunStore;
        let tmp = tempfile::tempdir().unwrap();
        let run_dir = tmp.path().join(".wingman/autonomous/r1");

        let mut store = RunStore::create(&run_dir, "r1", "goal", "abc", "wingman/auto/r1")
            .await
            .unwrap();
