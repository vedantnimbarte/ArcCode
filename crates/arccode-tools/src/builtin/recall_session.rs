...
//! (cross-project, served from `~/.arccode/sessions.db`).

use std::sync::Arc;

use crate::{Tool, ToolCtx};
use arccode_core::{ToolOutcome, ToolSpec};
use arccode_rag::{Embedder, IndexStore};
...
impl Tool for RecallSession {
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Search across all past arccode sessions (cross-project) for \
    ...
    async fn run(&self, args: Value, _ctx: &ToolCtx) -> ToolOutcome {
        ...
        let hits = match arccode_learn::session_index::search_sessions(
            &self.store,
            &*self.embedder,
            &args.query,
            limit,
