...
//!
//! On disk, every run lives under `<project>/.arccode/autonomous/<run-id>/`:
//!
//! ```text
//! <run-id>/
...
/// reattach to one already on disk (e.g. `arccode pilot resume`).
pub struct RunStore {
    dir: PathBuf,
    log: tokio::fs::File,
    state: RunState,
...
mod tests {
    ...
    async fn create_appends_run_start() {
        ...
        assert_eq!(store.state().integration_branch, "arccode/auto/r1");
    ...
    async fn append_then_reload_yields_same_state() {
        ...
        store
            .append(Event::TaskCreate {
                ...
                deps: vec![],
                writes: vec!["crates/arccode-cli/src/main.rs".into()],
                acceptance: vec![],
        ...
        store
            .append(Event::TaskAssign {
                ...
                id: "t1".into(),
                agent: "agent-1".into(),
                worktree: ".arccode/worktrees/auto-r1-t1".into(),
            })
            .await
            .unwrap();
