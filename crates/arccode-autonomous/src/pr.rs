...
pub fn render_pr_body(state: &RunState) -> String {
    ...
    let _ = writeln!(
        s,
        "_Opened by arccode pilot. Run id: `{}`. Base: `{}`._",
        state.run_id,
...
mod tests {
    ...
    fn sample_state() -> RunState {
        ...
        t1.outcome = Some(TaskOutcome {
            ...
            files_changed: vec!["crates/arccode-tui/src/composer.rs".into()],
        });
        s.tasks.push(t1);
        ...
        t2.outcome = Some(TaskOutcome {
            ...
            files_changed: vec!["crates/arccode-tui/src/theme.rs".into()],
        });
        t2.deps = vec!["t1".into()];
        s.tasks.push(t2);
        s.totals.usd = 0.42;
    ...
    fn pr_body_contains_goal_and_task_summaries() {
        ...
        assert!(body.contains("crates/arccode-tui/src/theme.rs"));
    ...
    fn parse_github_remote_handles_ssh_and_https() {
        assert_eq!(
            parse_github_remote("git@github.com:vedant/arccode.git"),
            Some(("vedant".into(), "arccode".into())),
        );
        assert_eq!(
            parse_github_remote("https://github.com/vedant/arccode.git"),
            Some(("vedant".into(), "arccode".into())),
        );
        assert_eq!(
            parse_github_remote("https://github.com/vedant/arccode/"),
            Some(("vedant".into(), "arccode".into())),
        );
    ...
    async fn gh_present_path_creates_pr_and_emits_events() {
        ...
        runner.respond(
            ...
            ok("https://github.com/vedant/arccode/pull/42\n"),
        );
        ...
        assert_eq!(outcome.url, "https://github.com/vedant/arccode/pull/42");
    ...
    async fn gh_missing_path_falls_back_to_push_and_compare_url() {
        ...
        runner.respond(
            ...
            ok("git@github.com:vedant/arccode.git\n"),
        );
        ...
        assert!(!outcome.created_by_gh);
        assert!(outcome.url.contains("github.com/vedant/arccode"));
        assert!(outcome.url.contains("compare/main...arccode/auto/r1"));
