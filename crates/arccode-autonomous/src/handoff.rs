...
pub fn render(packet: &HandoffPacket<'_>) -> String {
    ...
    out.push_str("## State\n\n");
    out.push_str(&format!(
        "Worktrees preserved. Resume with: `arccode pilot resume {}`.\n",
        s.run_id
    ));
...
mod tests {
    ...
    fn render_includes_goal_tier_and_resume_line() {
        ...
        assert!(md.contains("arccode pilot resume 2026-05-29-r1"));
    }
    ...
    fn write_packet_creates_file() {
        let dir = std::env::temp_dir().join(format!("arccode-handoff-{}", std::process::id()));
