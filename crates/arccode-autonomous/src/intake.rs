...
/// Strip a leading trigger token (e.g. `@arccode`, `/arccode`) from a
...
/// daemon points this at `.arccode/inbox/`. Files are read in sorted order
...
mod tests {
    ...
    fn strip_trigger_handles_mention() {
        assert_eq!(
            strip_trigger("@arccode fix the parser", "@arccode"),
            Some("fix the parser".to_string())
        );
        assert_eq!(
            strip_trigger("/arccode: add a flag", "/arccode"),
    ...
    fn strip_trigger_rejects_missing_trigger() {
        assert_eq!(strip_trigger("just a normal comment", "@arccode"), None);
    }

    #[test]
    fn strip_trigger_rejects_empty_body() {
        assert_eq!(strip_trigger("@arccode   ", "@arccode"), None);
    }
    ...
    fn normalize_returns_none_on_missing_trigger() {
        assert!(normalize(Channel::Slack, "hi there", Some("x"), Some("@arccode"), &[]).is_none());
    }
    ...
    fn scan_inbox_reads_goal_files() {
        let dir = std::env::temp_dir().join(format!("arccode-inbox-{}", std::process::id()));
    ...
    fn scan_inbox_missing_dir_is_empty() {
        let dir = std::env::temp_dir().join("arccode-no-inbox-xyz");
        let _ = std::fs::remove_dir_all(&dir);
